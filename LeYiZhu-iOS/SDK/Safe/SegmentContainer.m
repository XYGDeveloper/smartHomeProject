...
- (id)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame]) {
        ...
        self.topBarHeight = 0;//顶部选项条默认高度
        self.topBar.backgroundColor = [UIColor colorWithRed:67/255.0f green:67/255.0f blue:67/255.0f alpha:1.0f];
        self.itemPadding = 20;//项与项之间默认距离
        self.allowGesture = YES;//默认可以通过滑动来切换选项
        self.indicatorOffset = 10;//选项选中时标记的红条比文字长出的长度(效果图标记为10)
        self.indicatorHeight = 10;//标记选中的红线的高度
        self.indicatorView.image = [UIImage imageNamed:@"line"];
        self.bottomLineHeight = 0;//分隔线默认高度
...
- (UIButton *)itemButtonForIndex:(NSUInteger)index
{
    if (self.delegate && index < self.itemCount && [self.delegate respondsToSelector:@selector(segmentContainer:titleForItemAtIndex:)]) {
        ...
        button.backgroundColor = [UIColor colorWithRed:67/255.0f green:67/255.0f blue:67/255.0f alpha:1.0f];
        [button sizeToFit];
