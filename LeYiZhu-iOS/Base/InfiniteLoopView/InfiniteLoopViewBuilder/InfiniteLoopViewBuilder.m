...
- (void)startLoopAnimated:(BOOL)animated {
    ...
    self.loopView.index = self.initialIndex;
...
static CustomNodeStateView * extracted(InfiniteLoopViewBuilder *object) {
    return object.nodeViewTemplate;
}
...
- (void)managerNodeViewsContentView {
    ...
    if (self.models.count) {
        ...
        self.nodeViewsContentView.alpha = 1.0f;
        ...
        if (self.position == kNodeViewBottom || self.position == kNodeViewBottomLeft || self.position == kNodeViewBottomRight ||
            ...
            for (int i = 0; i < self.models.count; i++) {
                ...
                CustomNodeStateView *stateView = [[extracted(self).class alloc] initWithFrame:CGRectMake(0, i * height, width, height)];
