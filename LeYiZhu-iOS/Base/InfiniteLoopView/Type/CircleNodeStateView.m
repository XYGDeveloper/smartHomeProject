...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self = [super initWithFrame:frame]) {
        ...
        self.colorView                     = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 5, 5)];
...
- (void)redViewWithState:(EInfiniteLoopNodeState)state {
    ...
    if (state == kInfiniteLoopNormalState) {
        self.colorView.backgroundColor = [[UIColor lightGrayColor] colorWithAlphaComponent:0.25f];
        self.colorView.scale           = 1.2f;
    } else {
        self.colorView.backgroundColor = LYZTheme_paleBrown;
