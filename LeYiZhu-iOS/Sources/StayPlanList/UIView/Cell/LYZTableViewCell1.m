...
@end
@implementation LYZTableViewCell1
...
//        self.bgview.image = [UIImage imageNamed:@"comment"];
        self.bgview = [[UILabel alloc]init];
        self.bgview.numberOfLines = 0;
        self.bgview.textColor = [UIColor colorWithRed:102/255.0f green:102/255.0f  blue:102/255.0f  alpha:1.0f];
        self.bgview.font = [UIFont fontWithName:@"PingFangSC-Regular" size:14];
        self.bgview.textAlignment = NSTextAlignmentLeft;
...
- (void)cellDataWithModel:(LYZcommentDetailModel *)model
{
    ...
    self.bgview.text = [NSString stringWithFormat:@"回复:%@",model.reply];
    ...
    CGFloat replyHeight = [self contentHeight:model.reply];
    [self.bgview mas_updateConstraints:^(MASConstraintMaker *make) {
        make.left.mas_equalTo(self.contentLabel.mas_left);
        make.right.mas_equalTo(self.contentLabel.mas_right);
        make.top.mas_equalTo(self.roomtype.mas_bottom).mas_equalTo(4);
        make.bottom.mas_equalTo(0);
    }];
//    [self.bgview mas_updateConstraints:^(MASConstraintMaker *make) {
