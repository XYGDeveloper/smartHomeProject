...
#define kForegroundColor   [UIColor colorWithHexString:@"#CDBCA7"]//默认滑过颜色
#define kBackgroundColor [UIColor clearColor] //默认未滑过颜色
#define kThumbColor [UIColor whiteColor] //默认Thumb颜色
#define kBorderColor  LYZTheme_PinkishGeryColor //默认边框颜色
#define kThumbW 68 //默认的thumb的宽度
...
- (void)setupUI{
    ...
    _label.textColor =LYZTheme_warmGreyFontColor;
    ...
    _thumbImageView.layer.borderColor = LYZTheme_paleBrown.CGColor;
...
//    CGFloat thunmbW = self.thumbImage ? self.thumbImage.size.width : kThumbW;
    ...
    //修正
    p.x -= self.startX;
    if (p.x > kSliderW - thunmbW) {
        p.x = kSliderW - thunmbW;      }
    if (p.x < 0) {
        p.x = 0;
    }
    if (self.finishImage) {
    ...
    self.value = p.x  / (kSliderW - kThumbW);
    
    
    _foregroundView.frame = CGRectMake(0, 0, point.x + kBorderWidth, kSliderH);
    
    
    if (_foregroundView.frame.size.width <= kBorderWidth) {
        _thumbImageView.frame = CGRectMake(kBorderWidth, kBorderWidth, thunmbW, _foregroundView.frame.size.height- kBorderWidth);
        
    }else if (_foregroundView.frame.size.width >= (kSliderW - kThumbW  +2* kCornerRadius)) {
        _thumbImageView.frame = CGRectMake(kSliderW - kThumbW -2* kBorderWidth, kBorderWidth, thunmbW, _foregroundView.frame.size.height - 2 * kBorderWidth );
        
    }else{
        _thumbImageView.frame = CGRectMake(_foregroundView.frame.size.width - kCornerRadius -kBorderWidth , kBorderWidth, thunmbW, _foregroundView.frame.size.height-kBorderWidth*2);
    }
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event{
    ...
    NSLog(@"touch begin point ------->%f",point.x);
    self.startX = point.x;
//    [self fillForeGroundViewWithPoint:point];
}
