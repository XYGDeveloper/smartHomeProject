...
{
<<<<<<< HEAD
    NSString *appUserID = [[LoginManager instance]getUserInfo_isPresentLoginVC:NO].appUserID;
    ...
    [[LYZNetWorkEngine sharedInstance]userLogoutAppUserID:appUserID versioncode:VersionCode devicenum:DeviceNum fromtype:FromType block:^(int event, id object) {
        ...
        if(event == 1){
            UserLogoutResponse *logoutResponse = (UserLogoutResponse*)object;
            [[LoginManager instance]logout];
//            [LoginManager instance].isLogin = NO;
           JGProgressHUD *hud =  [Public hudWhenSuccessWithMsg:@"注销成功！"] ;
            [hud showInView:self.view animated:YES];
            [hud dismissAfterDelay:1.5f];
            
        }else{
            NSLog(@"exitBtnClick failure : %@" , object);
        }
...
>>>>>>> e962015ceadc04b35e951e2642541b36b5756c80
}
