...
//    [self.instructionBtn setTitle:@"不可用说明" forState:UIControlStateNormal];
    ...
    self.couponDescriptionLabel.font = [UIFont fontWithName:LYZTheme_Font_Regular size:13];
    self.couponDescriptionLabel.numberOfLines = 0;
...
//        self.couponNameLabel.attributedText = couponNameAttri;
        self.couponNameLabel.text = coupon;
...
//         NSString *coupon = [NSString stringWithFormat:@"%1.f%@折 折扣券",discout,@"%@"];
    ...
    self.couponDescriptionLabel.y = self.couponNameLabel.bottom + 7;
    self.couponDescriptionLabel.width = SCREEN_WIDTH - 2*self.couponNameLabel.x;
    [self.couponDescriptionLabel sizeToFit];
    self.deadlineLabel.textColor = isVaild ? LYZTheme_warmGreyFontColor: RGB(201, 201, 201);
    self.couponNameLabel.textColor = isVaild ?  RGB(208, 2, 27): RGB(201, 201, 201);
    self.couponStatusLabel.textColor = isVaild ? LYZTheme_warmGreyFontColor : RGB(201, 201, 201);
