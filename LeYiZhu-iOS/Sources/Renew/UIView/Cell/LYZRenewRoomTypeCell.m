...
@interface LYZRenewRoomTypeCell ()
...
static CGFloat _RenewRoomInfoCellHeight = 60.0f;

@implementation LYZRenewRoomTypeCell
...
//
//- (void)buildSubview {
//    self.roomTypeLabel = [[UILabel alloc] initWithFrame:CGRectMake(DefaultLeftSpace, 0, SCREEN_WIDTH/2.0 ,   LYZRenewRoomTypeCell.cellHeight)];
//    self.roomTypeLabel.font = [UIFont fontWithName:@"PingFangSC-Regular" size:14.0f];
//    self.roomTypeLabel.textColor =LYZTheme_warmGreyFontColor;
//    [self addSubview:self.roomTypeLabel];
//
//    self.priceLabel = [[UILabel alloc] initWithFrame:CGRectMake(SCREEN_WIDTH -DefaultLeftSpace - SCREEN_WIDTH/2.0, 0, SCREEN_WIDTH/2.0 ,   LYZRenewRoomTypeCell.cellHeight)];
...
//        NSString *price = dic[@"vipprice"];
//
//        BOOL isExact = [dic[@"exact"] isEqualToString:@"Y"] ? YES : NO;
//        if (isExact) {
//            self.priceLabel.text = [NSString stringWithFormat:@"￥%@/天",price];
//        }else{
//            NSDictionary * attriBute1 = @{NSForegroundColorAttributeName:LYZTheme_warmGreyFontColor,NSFontAttributeName:[UIFont fontWithName:LYZTheme_Font_Regular size:14]};
...
//
//    _RenewRoomInfoCellHeight = cellHeight;
//}
//
//+ (CGFloat)cellHeight {
//
//    return _RenewRoomInfoCellHeight;
//}
...
- (void)loadContent {
    if (self.data && [self.data isKindOfClass:[NSDictionary class]]) {
        ...
        self.roomTypeLabel.centerY = _RenewRoomInfoCellHeight/2.0;
        ...
        self.tagLabel0.y = self.roomTypeLabel.centerY + 2.5;
        ...
        if ([price isEqualToString:vipprice]) {
            if (isExact) {
                self.priceLabel.text = [NSString stringWithFormat:@"￥%@/天",price];
                [self.priceLabel sizeToFit];
                self.priceLabel.right = SCREEN_WIDTH -DefaultLeftSpace;
                self.priceLabel.centerY = _RenewRoomInfoCellHeight/2.0;
            }else{
                NSString *original =  [NSString stringWithFormat:@"￥%@ ",price];
            ...
            self.vipTagLabel.centerY = _RenewRoomInfoCellHeight /2.0;
