...
- (instancetype)initWithFrame:(CGRect)frame{
    if (self = [super initWithFrame:frame]) {
        ...
        [self setTitleColor:RGB(29, 29, 38) forState:UIControlStateNormal];
        [self setTitleColor:[UIColor whiteColor] forState:UIControlStateSelected];
        [self setTitleColor:[UIColor grayColor] forState:UIControlStateDisabled];
        self.titleLabel.font = [UIFont fontWithName:@".SF NS Display" size:14.0f];
        [self setImage:[UIImage imageNamed:@"circle_11"] forState:UIControlStateSelected];
        [self setTitle:@"入住" forState:UIControlStateSelected];
...
-(void)setManager:(ZYCalendarManager *)manager{
    ...
    if (_manager.selectionType == ZYCalendarSelectionTypeSingle) {
          [self setTitle:@"退房" forState:UIControlStateSelected];
    }else{
          [self setTitle:@"入住" forState:UIControlStateSelected];
    }
}
...
- (void)changeState {
    if (_manager.selectedStartDay && _manager.selectedEndDay) {
        ...
        if ([_manager.helper date:_date isTheSameDayThan:_manager.selectedStartDay.date]) {
            [self setBackgroundImage:[UIImage imageNamed:@"backImg_start0"]
            ...
            [self setBackgroundImage:[UIImage imageNamed:@"backImg_end0"]
                            forState:UIControlStateSelected];
            [self setTitle:@"退房" forState:UIControlStateSelected];
//            [self setImage:[UIImage imageNamed:@"circle_11"] forState:UIControlStateSelected];
            [self setTitleColor:[UIColor whiteColor] forState:UIControlStateSelected];
...
- (void)setSelectColor {
    if ([_manager.helper date:_date isEqualOrAfter:_manager.selectedStartDay.date andEqualOrBefore:_manager.selectedEndDay.date]) {
        ...
        if ([_manager.helper date:_manager.selectedStartDay.date isTheSameMonthThan:_manager.selectedEndDay.date]) {
            if (self.enabled) {
                self.backgroundColor = LYZTheme_paleBrown;
...
- (void)setDate:(NSDate *)date {
    ...
    if (self.enabled) {
        ...
        if ([_manager.helper date:_date isTheSameDayThan:_manager.date] && self.enabled) {
            [self setImage:[UIImage imageNamed:@"circle_cir0"] forState:UIControlStateNormal];
        }
