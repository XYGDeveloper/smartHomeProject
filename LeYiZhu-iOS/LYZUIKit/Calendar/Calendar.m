...
@interface Calendar () <FSCalendarDataSource,FSCalendarDelegate,FSCalendarDelegateAppearance>
...
//    calendar.appearance.titleOffset = CGPointMake(0, -25);
    ...
    NSDate *defaultCheckoutDate;
        if ([self isBetweenFromHour:0 toHour:6]) {
            defaultCheckinDate = [NSDate  dateYesterday];
            defaultCheckinDate = [NSDate date];
        }else{
            defaultCheckinDate = [NSDate date];
            defaultCheckoutDate = [NSDate dateTomorrow];
        }
    
    self.date1 =defaultCheckinDate ;
    self.date2 = defaultCheckoutDate;
...
//    confirmBtn.center = bottomView.center;
    ...
    confirmBtn.backgroundColor = LYZTheme_paleBrown;
...
- (void)calendar:(FSCalendar *)calendar didSelectDate:(NSDate *)date atMonthPosition:(FSCalendarMonthPosition)monthPosition
{
    ...
    if (calendar.swipeToChooseGesture.state == UIGestureRecognizerStateChanged) {
        ...
        if (self.date2) {
            ...
            if ([IICommons compareOneDay:date withAnotherDay:self.date1] <= 0) {
