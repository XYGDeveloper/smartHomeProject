...
- (UIButton *)cancelBtn{
    if (!_cancelBtn) {
        ...
        [_cancelBtn setTitleColor:LYZTheme_warmGreyFontColor
                         forState:UIControlStateNormal];
...
- (UIButton *)sureBtn{
    if (!_sureBtn) {
        ...
        [_sureBtn setTitle:@"完成"
                  forState:UIControlStateNormal];
        [_sureBtn setTitleColor:LYZTheme_paleBrown
                       forState:UIControlStateNormal];
...
- (void)showOrHide:(BOOL)isShow{
    ...
    [UIView animateWithDuration:0.5 animations:^{
        ...
        if (isShow) {
            selfkY = [UIScreen mainScreen].bounds.size.height - 266 -64;
        }
        else {
            selfkY = [UIScreen mainScreen].bounds.size.height - 64;
        }
        self.frame = CGRectMake(0,selfkY, self.bounds.size.width,266);
